...
$(document).ready(function(){ //페이지가 준비되면 function을 실행해라
	...
	$("#btnReg").on("click", function(){
		...
		if(emptCheck.test($("#userId").val())){
			...
			alert("사용자 아이디는 공백을 포함할 수 없습니다.");  //아이디에 공백있으면 경고창 띄우고 값 날려주기
			$("#userId").val("");
			$("#userId").focus();
			return;
		}
		...
		$.ajax({
			...
			data:{
				userId:$("#userId").val()  //자바스크립?HTML로 따지면 Id=sist&pw=1111처럼 넘기는 값
			},
			datatype:"JSON", //여기까지가 서버에서 실행하라고 요청
			
			//obj는 호출당해서 실행한곳에서 호출곳으로 던져준값 
			success:function(obj){ //성공했을 때 호출함 ,success:function부터error:function 까지는 클라이언트에서 실행되는 거임
				//위에서 호출했던 userIdCheckAJax.jsp 에서 {\"flag\":0}가 그대로  obj 안으로 들어오는 것임
				...
				//{\"flag\":0}을 parse()를 이용해서 JSON형태로 변환해 data변수에 대입
